<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8" />
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <title>House Leading - Post Page</title>
    <link rel="icon" type="image/png" href="images/logo.png" />    
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <link rel="stylesheet" type="text/css" media="screen" href="css/style.css" />
    <script src="js/script.js"></script>
  </head>

  <body onload="init()">
    <!-- Header -->
    <?php include 'header.php'; ?>

    <!-- Banner Start -->
    <div class="banner-section">
      <h1>PROPERTIES</h1>
    </div>
    <!-- Banner End -->

    <!-- Further Content start -->
    <article>
      <!-- Latest Posts Container Start -->
      <div class="latest-posts-container">

        <div class="container">

        <div class="tabs">
          <button class="tab-links btn-primery">House</button>
          <button class="tab-links btn-primery">Appartment</button>
        </div>

          <div class="latest-posts-wrapper">
             <!-- Card 1 End-->
             <div class="card">
                <div class="card-head">
                  <img src="images/feature1.jpg" alt="feature image">
                  <div class="cost">
                    <p class="cost-btn">
                      $954 890
                    </p>
                  </div>
                </div>

                <div class="card-body">
                  <h5>Vital Villas</h5>
                  <p class="location">
                    <i class="icon-hl-gps"></i> Upper Road 3411, no.34 CA
                  </p>

                  <p class="description">
                  Integer nec bibendum lacus. Suspendisse dictum enim sit amet libero malesuada.
                  <a href="single-post.php" class="more-link">Read More</a>
                  </p>

                  <div class="property-meta-data flex-d">
                    <div class="new-tag flex-d">
                      <i class="icon-hl-new"></i>
                    </div>

                    <div class="bathroom flex-d">
                      <i class="icon-hl-bathroom"></i>
                      <span class="bathroom-quantity">2</span>
                    </div>
                    <div class="bedroom flex-d">
                      <i class="icon-hl-bed"></i>
                      <span class="rooms-quantity">2</span>
                    </div>
                    <div class="space flex-d">
                      <i class="icon-hl-resize"></i>
                      <span class="space-measurement">120 sq ft</span>
                      
                    </div>
                  </div>  
                </div>
              </div>
          <!-- Card 1 End -->

          <!-- Card 2 End-->
          <div class="card">
                <div class="card-head">
                  <img src="images/feature1.jpg" alt="feature image">
                  <div class="cost">
                    <p class="cost-btn">
                      $954 890
                    </p>
                  </div>
                </div>

                <div class="card-body">
                  <h5>Vital Villas</h5>
                  <p class="location">
                    <i class="icon-hl-gps"></i> Upper Road 3411, no.34 CA
                  </p>

                  <p class="description">
                  Integer nec bibendum lacus. Suspendisse dictum enim sit amet libero malesuada.
                  <a href="single-post.php" class="more-link">Read More</a>
                  </p>

                  <div class="property-meta-data flex-d">
                    <div class="new-tag flex-d">
                      <i class="icon-hl-new"></i>
                    </div>

                    <div class="bathroom flex-d">
                      <i class="icon-hl-bathroom"></i>
                      <span class="bathroom-quantity">2</span>
                    </div>
                    <div class="bedroom flex-d">
                      <i class="icon-hl-bed"></i>
                      <span class="rooms-quantity">2</span>
                    </div>
                    <div class="space flex-d">
                      <i class="icon-hl-resize"></i>
                      <span class="space-measurement">120 sq ft</span>
                      
                    </div>
                  </div>  
                </div>
              </div>
          <!-- Card 2 End -->

          <!-- Card 3 End-->
          <div class="card">
                <div class="card-head">
                  <img src="images/feature1.jpg" alt="feature image">
                  <div class="cost">
                    <p class="cost-btn">
                      $954 890
                    </p>
                  </div>
                </div>

                <div class="card-body">
                  <h5>Vital Villas</h5>
                  <p class="location">
                    <i class="icon-hl-gps"></i> Upper Road 3411, no.34 CA
                  </p>

                  <p class="description">
                  Integer nec bibendum lacus. Suspendisse dictum enim sit amet libero malesuada.
                  <a href="single-post.php" class="more-link">Read More</a>
                  </p>

                  <div class="property-meta-data flex-d">
                    <div class="new-tag flex-d">
                      <i class="icon-hl-new"></i>
                    </div>

                    <div class="bathroom flex-d">
                      <i class="icon-hl-bathroom"></i>
                      <span class="bathroom-quantity">2</span>
                    </div>
                    <div class="bedroom flex-d">
                      <i class="icon-hl-bed"></i>
                      <span class="rooms-quantity">2</span>
                    </div>
                    <div class="space flex-d">
                      <i class="icon-hl-resize"></i>
                      <span class="space-measurement">120 sq ft</span>
                      
                    </div>
                  </div>  
                </div>
              </div>
          <!-- Card 3 End -->

          <!-- Card 4 End-->
          <div class="card">
                <div class="card-head">
                  <img src="images/feature1.jpg" alt="feature image">
                  <div class="cost">
                    <p class="cost-btn">
                      $954 890
                    </p>
                  </div>
                </div>

                <div class="card-body">
                  <h5>Vital Villas</h5>
                  <p class="location">
                    <i class="icon-hl-gps"></i> Upper Road 3411, no.34 CA
                  </p>

                  <p class="description">
                  Integer nec bibendum lacus. Suspendisse dictum enim sit amet libero malesuada.
                  <a href="single-post.php" class="more-link">Read More</a>
                  </p>

                  <div class="property-meta-data flex-d">
                    <div class="new-tag flex-d">
                      <i class="icon-hl-new"></i>
                    </div>

                    <div class="bathroom flex-d">
                      <i class="icon-hl-bathroom"></i>
                      <span class="bathroom-quantity">2</span>
                    </div>
                    <div class="bedroom flex-d">
                      <i class="icon-hl-bed"></i>
                      <span class="rooms-quantity">2</span>
                    </div>
                    <div class="space flex-d">
                      <i class="icon-hl-resize"></i>
                      <span class="space-measurement">120 sq ft</span>
                      
                    </div>
                  </div>  
                </div>
              </div>
          <!-- Card 4 End -->

          <!-- Card 5 End-->
          <div class="card">
                <div class="card-head">
                  <img src="images/feature1.jpg" alt="feature image">
                  <div class="cost">
                    <p class="cost-btn">
                      $954 890
                    </p>
                  </div>
                </div>

                <div class="card-body">
                  <h5>Vital Villas</h5>
                  <p class="location">
                    <i class="icon-hl-gps"></i> Upper Road 3411, no.34 CA
                  </p>

                  <p class="description">
                  Integer nec bibendum lacus. Suspendisse dictum enim sit amet libero malesuada.
                  <a href="single-post.php" class="more-link">Read More</a>
                  </p>

                  <div class="property-meta-data flex-d">
                    <div class="new-tag flex-d">
                      <i class="icon-hl-new"></i>
                    </div>

                    <div class="bathroom flex-d">
                      <i class="icon-hl-bathroom"></i>
                      <span class="bathroom-quantity">2</span>
                    </div>
                    <div class="bedroom flex-d">
                      <i class="icon-hl-bed"></i>
                      <span class="rooms-quantity">2</span>
                    </div>
                    <div class="space flex-d">
                      <i class="icon-hl-resize"></i>
                      <span class="space-measurement">120 sq ft</span>
                      
                    </div>
                  </div>  
                </div>
              </div>
          <!-- Card 5 End -->

          <!-- Card 6 End-->
          <div class="card">
                <div class="card-head">
                  <img src="images/feature1.jpg" alt="feature image">
                  <div class="cost">
                    <p class="cost-btn">
                      $954 890
                    </p>
                  </div>
                </div>

                <div class="card-body">
                  <h5>Vital Villas</h5>
                  <p class="location">
                    <i class="icon-hl-gps"></i> Upper Road 3411, no.34 CA
                  </p>

                  <p class="description">
                  Integer nec bibendum lacus. Suspendisse dictum enim sit amet libero malesuada.
                  <a href="single-post.php" class="more-link">Read More</a>
                  </p>

                  <div class="property-meta-data flex-d">
                    <div class="new-tag flex-d">
                      <i class="icon-hl-new"></i>
                    </div>

                    <div class="bathroom flex-d">
                      <i class="icon-hl-bathroom"></i>
                      <span class="bathroom-quantity">2</span>
                    </div>
                    <div class="bedroom flex-d">
                      <i class="icon-hl-bed"></i>
                      <span class="rooms-quantity">2</span>
                    </div>
                    <div class="space flex-d">
                      <i class="icon-hl-resize"></i>
                      <span class="space-measurement">120 sq ft</span>
                      
                    </div>
                  </div>  
                </div>
              </div>
               <!-- Card 6 End -->
          </div>

        </div>

      </div>
      <!-- Latest Posts Container End -->
    </article>
    <!-- Further Content End -->


    
    <!-- Footer -->
    <?php include 'footer.php'; ?>
  </body>
</html>